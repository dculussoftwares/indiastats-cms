...
// ── Second Place Card ─────────────────────────────────────────────────────────
function SecondPlaceGauge({
  d,
  seats,
  selected,
  onClick,
}: {
  d: { party: string; count: number; total: number; secondPct: number }
  seats: number
  selected: boolean
  onClick: () => void
}) {
  ...
  const r = 40
  const sw = 9
  const pct = Math.min(Math.max(d.secondPct, 0), 100)
  ...
  return (
    ...
    >
      ...
      <svg viewBox="0 0 120 90" className="w-full max-w-[140px]">
        ...
        {fillPath && (
          <path d={fillPath} fill="none" stroke="#f59e0b" strokeWidth={sw} strokeLinecap="round" />
        )}
        <text x={cx} y={cy - 8} textAnchor="middle" fontSize="13" fontWeight="800" fill="#f59e0b">
          {pct}%
        </text>
...
function SecondPlaceCard({
  ...
  const { secondPlaces, partyVoteShares } = data
  if (!secondPlaces || secondPlaces.length === 0) return null
  ...
  const top6 = partyVoteShares
    .slice(0, 6)
    .map((pvs) => {
      const sp = secondPlaces.find((d) => d.party === pvs.party)
      return sp ? { ...sp, seats: pvs.seats } : null
    })
    .filter(Boolean)
    .sort((a, b) => b!.count - a!.count) as ((typeof secondPlaces)[0] & { seats: number })[]
    ...
    <Card>
      <CardContent className="pt-4 pb-4">
        <div className="flex items-start justify-between mb-4">
          <p className="text-xs text-muted-foreground">
            <span className="font-semibold text-foreground">{totalSecond}</span> second-place
            finishes recorded across all constituencies
          </p>
        </div>
          ...
          <div className="mt-4 border border-gray-100 dark:border-gray-800 rounded">
            <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 dark:bg-gray-800/50 rounded-t">
              ...
              <span className="text-xs text-muted-foreground ml-auto">
                {selectedData.assemblies.length} constituencies where candidate placed 2nd
              </span>
            </div>
            <div className="divide-y divide-gray-50 dark:divide-gray-800">
                ...
                return (
                  <div key={a.assemblyId} className="flex items-center gap-3 px-3 py-2">
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <span className="text-[10px] text-amber-500 font-semibold ml-1.5 tabular-nums">
                        {a.votePct}%
                      </span>
                    </div>
                  </div>
        ...
        <p className="text-[10px] text-muted-foreground mt-3 pt-2 border-t border-gray-100 dark:border-gray-800">
          Gauge shows % of a party&apos;s candidates who finished 2nd in their constituency. Click a
...
export function ElectionAnalysisClient({
  ...
  return (
    <div className="space-y-8 mt-4">
      ...
      {/* ⑦e Second Place Finishes */}
      {data.secondPlaces && data.secondPlaces.length > 0 && (
        <section id="second-place">
          <h2 className="text-lg font-bold border-l-4 border-red-600 pl-3 mb-4">
            2nd Place Finishes
          </h2>
          <SecondPlaceCard data={data} stateSlug={stateSlug} />
        </section>
      )}
