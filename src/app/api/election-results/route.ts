...
export async function GET(request: NextRequest) {
  try {
    ...
    const supportedYears = getStateByCode(stateCode)?.electionYears ?? []
    ...
    const candidatesByAssembly: Record<
      string,
      Array<{
        candidateName: string
        party: string
    ...
    return NextResponse.json({ error: 'Failed to fetch election results' }, { status: 500 })
  }
}
